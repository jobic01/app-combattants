...
# --- CONFIGURATION ---
st.set_page_config(page_title="Les Combattants Légendaires", page_icon="🥊", layout="wide")

# --- CONNEXION GOOGLE SHEETS ---
conn = st.connection("gsheets", type=GSheetsConnection)

# --- CHARGEMENT DES DONNÉES ---
# On utilise ttl=0 pour ne pas garder de cache trop longtemps (mise à jour immédiate)
def load_data():
...
try:
    ...
    st.error("Erreur de connexion au Google Sheet. Vérifiez les 'Secrets'.")
    st.stop()
...
# --- PAGE 3 : COMBATTANTS ---
    ...
    st.title("🥷 Liste des Combattants")
...
# --- PAGE 4 : ARBITRAGE (Le Formulaire) ---
    ...
    st.header("📝 Feuille de Match")
    st.warning("Attention : L'envoi mettra à jour le fichier Google Sheet directement.")
    ...
    with st.form("assaut_form"):
        ...
        with col1:
            ...
            chasses = st.number_input("Chassés", min_value=0, step=1, key="ch1")
            fouettes = st.number_input("Fouettés", min_value=0, step=1, key="fo1")
            poings = st.number_input("Directs/Crochets", min_value=0, step=1, key="po1")
            
        with col2:
            st.subheader("Adversaire (Rouge)")
            # On enlève le combattant 1 de la liste pour éviter qu'il se batte contre lui-même
        ...
        if submitted:
            # 1. Création de la nouvelle ligne de données
            # IMPORTANT : Les colonnes doivent correspondre à ton fichier Excel "LES ASSAUTS"
            ...
            new_row = pd.DataFrame([{
                ...
                "FOUETTES": fouettes,
                "DIRECTS": poings,
                # Ici on met une date ou un numéro d'assaut auto
                "ASSAUTS": f"Assaut {datetime.now().strftime('%d/%m %H:%M')}", 
                "TOTAL ASSAUT": 0 # Sera calculé par Google Sheet si tu as mis les formules
            }])
            
            # 2. Ajout au Google Sheet
            # On concatène l'ancien historique avec la nouvelle ligne
            ...
            st.success(f"Combat enregistré : {c1_name} vs {c2_name} !")
